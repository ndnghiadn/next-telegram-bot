...
// Additional simple commands for the commands class
BotCommands.prototype.timeCommand = async function (
  ...
  await bot.sendMessage(chatId, `🕐 *Current time:* ${currentTime}`, {
    parse_mode: "Markdown",
  });
};

BotCommands.prototype.echoCommand = async function (
  msg: TelegramBot.Message,
...
BotCommands.prototype.logsCommand = async function (
  msg: TelegramBot.Message,
): Promise<void> {
  const chatId = msg.chat.id;
  const userId = msg.from?.id;
...
BotCommands.prototype.detailedStatsCommand = async function (
  msg: TelegramBot.Message,
): Promise<void> {
  const chatId = msg.chat.id;
  const userId = msg.from?.id;
